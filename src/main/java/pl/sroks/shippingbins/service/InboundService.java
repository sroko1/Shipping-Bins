...
public class InboundService {
    ...
    public InboundDto updateInbound(int id, Map<Object, Object> fields) {
        Inbound inbound = findInboundInDB(id);
        fields.forEach((key, value) -> {
            Field field = ReflectionUtils.findField(Inbound.class, (String) key);
            if (field == null) {
                throw new FileDoesNotExistException("Field " + key + " does not exist");
            }
            field.setAccessible(true);
            ReflectionUtils.setField(field, inbound, value);
        });
        inboundRepository.save(inbound);
        return InboundConverter.convertToInboundDto(inbound);
    }
